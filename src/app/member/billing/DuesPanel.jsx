...
export function DuesPanel() {
  ...
  const { data: billing, reload } = useApi("/member/billing");
  const { data: rawPlots } = useApi("/member/plots");
  const plot = normalizeList(rawPlots)[0];
  const dues = billing?.summary?.totalDue ?? 0;
  // The invoice a slip opens for — the next outstanding one (else the latest).
  const upcoming = normalizeList(billing?.upcoming);
  const primarySlip = upcoming.find((i) => (i.balance ?? 0) > 0) ?? upcoming[0] ?? null;
  const me = {
    plotNo: plot?.plotNo ?? user?.plotNo ?? "—",
    sizeSqyd: plot?.sizeSqyd ?? 0,
    amountDue: dues,
    daysOverdue: 0,
  };
  ...
  // Executable UPI intent for the outstanding balance — scan to pay from any app.
  const upiUri = buildUpiUri({
    vpa: bank.upi,
    payeeName: bank.qrPayeeName || bank.accountName || settings.name,
    amount: dues,
    note: bank.qrNote ? `${bank.qrNote} ${me.plotNo}` : `Maintenance ${me.plotNo}`,
    reference,
  });

  return (
    <div className="animate-fade-in">
      <PageHeader title="Dues & Payments" subtitle={`Plot ${me.plotNo} · FY ${settings.fy}`} />
          ...
          <Card className="p-6">
            ...
            <div className="mt-5 rounded-xl bg-slate-50 p-4 text-sm">
              <div className="flex justify-between py-1">
                <span className="text-slate-500">Maintenance fee ({me.sizeSqyd} sqyd × ₹{settings.ratePerSqyd})</span>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              {primarySlip && (
                <Button variant="secondary" icon="receipt-indian-rupee" onClick={() => setSlipOpen(true)}>
                  View slip
                </Button>
              )}
          ...
          <Card>
            ...
            <div className="space-y-3 p-5 text-sm text-slate-600">
              <p>
                ...
                and common-area cleaning. The fee for FY {settings.fy} is{" "}
                <b>₹{settings.ratePerSqyd} per sqyd per year</b>.
              </p>
              <p>
                Your plot is <b>{me.sizeSqyd} sqyd</b>, so your annual fee is{" "}
                <b>{formatINR(me.sizeSqyd * settings.ratePerSqyd)}</b>.
                {settings.dueDate && (
                  <>
                    {" "}Pay before <b>{settings.dueDate}</b>
                    {settings.latePenaltyPct ? ` to avoid a ${settings.latePenaltyPct}% late penalty` : ""}.
                  </>
                )}
              </p>
            </div>
        ...
        <Card className="h-fit">
          ...
          <div className="p-5">
            <div className="grid place-items-center rounded-xl bg-brand-50 p-4">
              <div className="grid h-36 w-36 place-items-center rounded-xl bg-white p-2 shadow-sm">
                {bank.qrImageUrl || upiUri ? (
                  <PaymentQr imageUrl={bank.qrImageUrl} value={upiUri} size={120} />
                ) : (
                  <Icon name="qr-code" size={84} className="text-slate-300" />
                )}
              </div>
              <p className="mt-2 text-xs text-slate-500">
                {bank.qrImageUrl || upiUri ? "Scan to pay" : "UPI not configured yet"}
              </p>
              {bank.upi && (
                <p className="font-mono text-sm font-medium text-brand-700">{bank.upi}</p>
              )}
            </div>

            <div className="mt-4 divide-y divide-slate-100">
              <CopyRow label="Account name" value={bank.accountName} />
              <CopyRow label="Account number" value={bank.accountNo} />
      ...
      {/* Payment slip for the next outstanding invoice */}
      <PaymentSlipModal
        open={slipOpen}
