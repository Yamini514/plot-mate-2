...
const KIND_ICON = {
  ...
  notice: "megaphone", project: "hammer", support: "life-buoy",
...
export default function MemberNotificationsPage() {
  ...
  const { data: raw, meta, reload, loading } = useApi("/member/notifications", {
  ...
  const open = async (n) => {
    if (!n.read) {
      try { await api.post(`/member/notifications/${n.id}/read`, {}); reload(); } catch { /* ignore */ }
    }
  ...
  const markAll = async () => {
    setBusy(true);
    try { await api.post("/member/notifications/read-all", {}); toast("All marked read"); reload(); }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Notifications"
        subtitle="Updates on your payments, complaints, requests and notices"
          ...
          <ul className="divide-y divide-slate-100">
            {items.map((n) => (
              <li key={n.id}>
                <button
                  onClick={() => open(n)}
                  className={`flex w-full items-start gap-3 px-4 py-3 text-left transition-colors hover:bg-slate-50 ${n.read ? "" : "bg-brand-50/40"}`}
                >
