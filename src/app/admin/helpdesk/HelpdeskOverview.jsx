...
export function HelpdeskOverview() {
  ...
  const { data: s } = useApi("/admin/helpdesk/tickets/summary");
  const helpdeskStats = {
    total: s?.total ?? 0, open: s?.open ?? 0, inProgress: s?.inProgress ?? 0,
    resolved: s?.resolved ?? 0, closed: s?.closed ?? 0, escalatedCount: s?.escalated ?? 0,
    overdue: s?.overdue ?? 0, reopened: s?.reopened ?? 0,
    slaCompliance: s?.slaCompliance ?? 0, avgResolutionHrs: s?.avgResolutionHrs ?? 0,
  };
  const categoryTrend = s?.categoryDistribution ?? [];
  const statusDistribution = s?.statusDistribution ?? [];
  const staffPerformance = (s?.staffPerformance ?? []).map((x, i) => ({ ...x, color: ["#059669", "#0ea5e9", "#8b5cf6", "#f59e0b", "#14b8a6", "#ef4444"][i % 6] }));
  const slaComplianceTrend = (s?.slaByPriority ?? []).map((x, i) => ({ ...x, color: ["#ef4444", "#f97316", "#f59e0b", "#94a3b8"][i] }));
  const breachedCount = helpdeskStats.overdue;
      ...
      {breachedCount > 0 && (
        <div className="mb-6 flex flex-wrap items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-rose-800">{breachedCount} tickets have breached SLA</p>
