...
import { PageHeader, Breadcrumbs, Card, Button, Modal, Drawer, Field, inputClass, StatusBadge, EmptyState } from "@/components/ui";
...
export default function GuardSupportPage() {
  const toast = useToast();
  const { data: raw, reload } = useApi("/guard/support");
  ...
  const { data: detail, reload: reloadDetail } = useApi(openId ? `/guard/support/${openId}` : null);
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try { await api.post("/guard/support", form); toast("Support ticket raised"); setForm({ subject: "", description: "", priority: "medium" }); setOpen(false); reload(); }
  ...
  const sendReply = async () => {
    ...
    setBusy(true);
    try { await api.post(`/guard/support/${openId}/reply`, { body: reply.trim() }); setReply(""); reloadDetail(); }
  ...
  return (
    <div className="animate-fade-in">
      <Breadcrumbs items={[{ label: "PlotMate", href: "/guard" }, { label: "System" }, { label: "Support" }]} />
